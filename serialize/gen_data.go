...
func GenDataFile(dir string, table *model.DataTable) (bool, string, pref.FileDescriptor) {
	defer func() {
	...
	fd, err := utils.BuildDynamicType([]*model.DataTable{table})
	if err != nil {
		log.Printf("类型构建失败 类型:%s 详情:%s \n", table.TypeName, err.Error())
		return false, dtfileName, nil
	}
